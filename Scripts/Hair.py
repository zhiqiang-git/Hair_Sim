...
class Hair_DER:
    def __init__(
        ...
        # basic parameters
        self.num_rod = num_rod
        self.num = num_vertice
        self.dt = dt
        self.rho = rho
        self.r = r
        self.E = E
        ...
        for i in range(self.num_rod):
            for j in range(self.num):
                ...
                self.X[i, j] = X[i, j]
                self.Fixed[i, j] = Fixed[i, j]
                if j != self.num - 1:
                    self.Tao[i, j] = 0
                    self.Omega[i, j] = 0
                    self.Theta[i, j] = Theta[i, j]
    ...
    def Rotate(
        ...
        # v may be not normalized, t is a normalized vector
        # b = ti.math.normalize(b)
        ...
        for _ in range(1):
            ...
            for i in range(self.num_rod):
                for j in range(self.num - 1):
                    ...
                    if j == 0:
                        rand_vec = ti.Vector([0, 0, 1.0])  # a normalized vector
                        self.a1[i, j] = ti.math.cross(self.i_tangent[i, j], rand_vec)
                    else:
